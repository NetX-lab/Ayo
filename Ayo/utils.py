...
def fill_prompt_template_with_placeholdersname_approximations(prompt_template, input_kwargs):
    """
    ...
    for placeholder in placeholders:
        if placeholder in input_kwargs:
            ...
            if close_matches:
                print_warning(f"Use approximate matching: Placeholder '{placeholder}' is matched to input parameter '{close_matches[0]}'")
...
def check_unfilled_placeholders_in_prompt_template(prompt_template):
    ...
    placeholders = re.findall(r'\{([^{}]+)\}', prompt_template)
    if placeholders:
        raise ValueError(f"Prompt template is not complete without any unfilled placeholders")
    else:
        return True
