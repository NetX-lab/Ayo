...
def get_aggregator_config(node: Node, **kwargs) -> Dict:
    ...
    assert node.op_type == NodeOps.AGGREGATOR, f"node {node.name} is not an aggregator node"
    if node.parents[0].op_type == NodeOps.EMBEDDING:
        ...
        agg_config.update(
                    {
                       node.config.get("topk",{}) or node.config.get("top_k",{}) or node.config.get("k",{}) or 5
                    }
                )
        return agg_config
