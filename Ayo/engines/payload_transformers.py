...
from typing import Any, Dict, List, Union, Tuple, TYPE_CHECKING
import numpy as np
from enum import Enum
from vllm import SamplingParams

from Ayo.dags.node_commons import NodeOps
from Ayo.utils import check_prompt_template_and_placeholders_match, check_unfilled_placeholders_in_prompt_template, print_warning, fill_prompt_template_with_placeholdersname_approximations
...
class VectorDBTransformer(PayloadTransformer):
    ...
    def transform(self, node: 'Node') -> Tuple[RequestType, Any]:
        ...
        request_type = RequestType.INGESTION if node.op_type==NodeOps.VECTORDB_INGESTION else RequestType.SEARCHING
        ...
        if request_type == RequestType.INGESTION:
            ...
            for key, value in node.input_kwargs.items():
                if "embed" not in key.lower() and ("text" in key.lower() or "passage" in key.lower()):
                    texts = value
                    break
            ...
            if isinstance(embeddings, list):
                assert len(embeddings) == num_text_embeddings_pairs, "Number of embeddings must match number of texts"
                assert isinstance(embeddings[0], list), "Embeddings must be a list of lists"

            elif isinstance(embeddings, np.ndarray):
                assert len(embeddings) == num_text_embeddings_pairs, "Length of embeddings np array must match number of texts"
            ...
            for key, value in node.input_kwargs.items():
                if key.lower() in ["query_vectors", "query_vector", "query_embedding", "query_embeddings","queries_embeddings",'embeddings','embedding','query_vector_list','query_vector_lists','queries','query_list','query_lists']:
                    query_vectors = value
                    break
            ...
            if isinstance(query_vectors, list):
                if len(query_vectors) == 0:
                    raise ValueError(f"Empty query_vectors list for node {node.name} vector db searching")
                
                if isinstance(query_vectors[0], list):
                    # Convert list of lists to list of numpy arrays
                    try:
                        query_vectors = [np.array(query_vector, dtype=float) for query_vector in query_vectors]
                    except (ValueError, TypeError):
                        raise ValueError(f"Failed to convert query vectors to numpy arrays for node {node.name} vector db searching")
                    ...
                    for i, qv in enumerate(query_vectors):
                        if qv.ndim != 1:
                            raise ValueError(f"Query vector at index {i} has {qv.ndim} dimensions, expected 1D for node {node.name} vector db searching")
                
                else:
                    raise ValueError(f"Invalid query_vector format: expected list of lists or numpy arrays for node {node.name} vector db searching")
                ...
                if query_vectors.ndim == 1:
                    ...
                    raise ValueError(f"Query vector numpy array has {query_vectors.ndim} dimensions, expected 1D or 2D for node {node.name} vector db searching")
            
            else:
                raise ValueError(f"Invalid query_vector format: expected list or numpy array for node {node.name} vector db searching")

            #（query_vector, top_k） pair
            data = (query_vectors, top_k)

            assert top_k is not None, f"Missing top_k for node {node.name} vector db searching"
            assert query_vectors is not None, f"Missing query_vector for node {node.name} vector db searching"
...
class RerankerTransformer(PayloadTransformer):
    def transform(self, node: 'Node') -> Dict:
        ...
        for key, value in node.input_kwargs.items():
            if key.lower() in ["query", "queries", "query_text", "query_texts", "query_texts_list"]:
                query = value
            elif key.lower() in ['text','texts',"passages", "documents", "context","document","contexts","docs","search_results","search_result_list","search_result_lists",'searching_results','searching_result_list','searching_result_lists']:
                passages = value
        ...
        if isinstance(query, list):
            if len(query) == 1 and isinstance(query[0], str):
                ...
                raise ValueError(f"Query is a list for node {node.name} reranking, only string is supported. please check the input_kwargs")
            ...
            if isinstance(passages[0], list):
                ...
                for i, passage_list in enumerate(passages):
                    if not isinstance(passage_list, list):
                        raise ValueError(f"Inconsistent passages format for node {node.name} reranking")
                    
                    # Skip empty lists
                    if len(passage_list) == 0:
                        raise ValueError(f"Empty passages list at index {i} for node {node.name} reranking")
                    ...
                    if passage_list and isinstance(passage_list[0], str):
                        ...
                        merged_passages.extend([str(p) for p in passage_list if p is not None])
                        ...
                        for key in passage_list[0].keys():
                            if key.lower() in ["text", "passage", "doc", "document", "context", "content", 'texts', 'passages', 'docs', 'documents', 'contexts']:
                                text_key = key
                                break
                        
                        if text_key is None:
                            raise ValueError(f"Missing text key in nested dict list for node {node.name} reranking")
                        
                        # Extract all texts at once
                        merged_passages.extend(str(p_dict[text_key]) for p_dict in passage_list)
                    else:
                        raise ValueError(f"Passages contains unsupported element types for node {node.name} reranking")
                ...
                for key in passages[0].keys():
                    if key.lower() in ["text","passage","doc","document","context","content",'texts','passages','docs','documents','contexts']:
                        text_key = key
                    elif key.lower() in ["score","similarity_score","cosine_score","cosine_similarity_score"]:
                        score_key = key
                    else:
                        raise ValueError(f"Invalid passage format for node {node.name} reranking")
                ...
                raise ValueError(f"Invalid passages format {type(passages[0])} for node {node.name} reranking")
            
            # ensure the processed list is not empty
            if len(passages) == 0:
                raise ValueError(f"No valid passages after processing for node {node.name} reranking")
            

        elif isinstance(passages, str):
            passages = [passages]
            logger.warning(f"Passages is a single string for node {node.name} reranking, convert to list")
        else:
...
class LLMTransformer(PayloadTransformer):
    def transform(self, node: 'Node') -> Dict[str, Any]:
        ...
        for key, value in node.config.items():
            if key.lower() in ["prompt_template", "prompt_templates",'template','templates']:
        ...
        if node.op_type in [NodeOps.LLM_PREFILLING, NodeOps.LLM_PARTIAL_PREFILLING,NodeOps.LLM_FULL_PREFILLING]:
            if potential_prompt is None and potential_prompt_template is None:
                raise ValueError(f"Missing prompt for node {node.name} llm prefilling-related operation {node.op_type}")
            else:
                if potential_prompt_template is not None:
                    # we should fill the prompt template with the input_kwargs
                    prompt= fill_prompt_template_with_placeholdersname_approximations(potential_prompt_template, node.input_kwargs)
                    ...
                    if not check_unfilled_placeholders_in_prompt_template(prompt):
                        raise ValueError(f"Prompt is not complete without any unfilled placeholders for node {node.name} llm prefilling-related operation {node.op_type}")
        ...
        sampling_params = SamplingParams(temperature=0.9, top_p=0.95,max_tokens=max_tokens,
                                         support_partial_output=support_partial_output,
                                         support_partial_prefilling=support_partial_prefilling)
        

        assert llm_internal_id != "", f"Missing llm_internal_id for node {node.name} llm"
        assert llm_op_type in [NodeOps.LLM_PREFILLING, NodeOps.LLM_PARTIAL_PREFILLING, NodeOps.LLM_FULL_PREFILLING, NodeOps.LLM_DECODING,NodeOps.LLM_PARTIAL_DECODING], f"Invalid llm_op_type for node {node.name} llm"
        
        if llm_op_type in [NodeOps.LLM_PREFILLING, NodeOps.LLM_PARTIAL_PREFILLING, NodeOps.LLM_FULL_PREFILLING]:
