...
class VectorDBRequest:
    ...
    data: Union[List[Tuple[Union[np.ndarray, list], str]], Tuple[Union[np.ndarray, list], int]]  # (embeddings, texts) for ingestion or (query_vector, top_k) for searching
...
class VectorDBEngine:
    ...
    async def submit_request(self,
                           ...
                           query_id: str,
                           request_type: RequestType,
                           data: Union[List[Tuple[np.ndarray, str]], Tuple[np.ndarray, int]]) -> None:
        ...
        logger.info(f"submit request in vector_db: {request.request_type} {request.request_id} {request.query_id}")
                ...
                while len(batch_requests) ==0:
                    try:
                        request = await asyncio.wait_for(
                            self.insert_queue.get(),
                            timeout=0.1
                        )
                        batch_requests.append(request)
                        batch_data.append(request.data)

                        logger.debug(f"vector_db insert batch_requests len: {len(batch_requests)}")
                ...
                for query_id, indices in query_groups.items():
                    ...
                    async with self.pool.acquire() as conn:
                        ...
                        if isinstance(group_data[0][0], list): 
                            ...
                            await conn.executemany(
                                f"INSERT INTO {table_name} (embedding, text) VALUES ($1, $2)",
                                [(embedding.tolist(), text) for embedding, text in group_data]
                            )
                        ...
                        logger.debug(f"Table {table_name} contains {count} records after ingest")
    ...
    async def _process_searches(self):
        ...
        while self.running:
            try:
                ...
                logger.debug(f"search in vector_db: {request.request_id} {request.query_id} {table_name}")
                ...
                async with self.pool.acquire() as conn:
                    ...
                    for query_vector in query_vectors:
                        ...
                        search_results = [
                            {
                                "text": record["text"],
                                "score": 1 - float(record["distance"])  # Convert distance to similarity score
                            }
                    ...
                    logger.debug(f"search time for {len(query_vectors)} vectors: {end - begin}")
                ...
                if len(all_results) == 1:
                    ...
                    for i, result in enumerate(search_results):
                        search_results[i] = sorted(result, key=lambda x: x["score"], reverse=True)[:top_k]
    ...
    async def shutdown(self):
        ...
        try:
            ...
            print(f"Successfully cleaned up all tables")
        except Exception as e:
