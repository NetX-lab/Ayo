    ...
    async def _process_requests(self):
        ...
        while self.running:
            try:
                ...
                try:
                    request = await asyncio.wait_for(self.request_queue.get(), timeout=0.005)
                    
                    
                    # Create a processing task and add it to the active task set
                    task = asyncio.create_task(self._handle_request(request, request.sampling_params))
    ...
    async def _handle_request(self, request, sampling_params):
        ...
        try:
            ...
            if request.llm_op_type == NodeOps.LLM_FULL_PREFILLING:
                ...
                logger.info(f"Time {formatted_time}, LLM engine full prefilling: {request.llm_op_type} llm_internal_id: {request.llm_internal_id}, prompt: {request.prompt}, sampling_params: {sampling_params}") 
            ...
            elif request.llm_op_type in [NodeOps.LLM_PARTIAL_PREFILLING, NodeOps.LLM_PREFILLING]:
                ...
                self.result_generator_tracker[request.llm_internal_id] = result_generator
                ...
                while True:
                    result_generator = self.result_generator_tracker[request.llm_internal_id]
                ...
                async for result in result_generator:
                    if result.finished:
                        ...
                        print(f"Result for decoding request {request.request_id}: {result.outputs[0].text}")
                    ...
                    try:
                        async for result in result_generator:
                            if result.outputs[0].partial_outputs:
                                ...
                                if len(result.outputs[0].partial_outputs) > partial_decode_idx:
                                    if self.scheduler_ref is not None:
                                        await self.scheduler_ref.on_result.remote(
                                            request.request_id,
                                            request.query_id,
                                            result.outputs[0].partial_outputs[partial_decode_idx]
                                        )
                                  
                                    if first_partial_result is None:
                                        first_partial_result=result.outputs[0].partial_outputs[partial_decode_idx]
                                        print(f"Time: {time.time()}, Partial output: {first_partial_result} for partial decoding idx: {partial_decode_idx}")
                    
                            if result.finished:
                                async with self.partial_results_lock:
                                    state['finished'] = True
                                print(f"final output: {result.outputs[0].text}")
                                del self.result_generator_tracker[request.llm_internal_id]
                                break
                    except Exception as e:
                ...
                # Other requests poll for results
                    ...
                    while True:
                        ...
                        async with self.partial_results_lock:
                            current_results = state['results'].copy() if state['results'] else []
                        ...
                        if len(current_results) > partial_decode_idx:
                            ...
                            print(f"Time: {time.time()}, Partial output: {current_results[partial_decode_idx]} for partial decoding idx: {partial_decode_idx}")
                            return
        ...
        if isinstance(self.engine, ray.actor.ActorHandle):
            try:
                if hasattr(self.engine, '_background_loop_unshielded'):
                    await ray.get(self.engine._background_loop_unshielded.cancel.remote())
                ...
                try:
                    await ray.get(self.engine.engine._run_workers_async.remote("terminate", get_all_outputs=False))
                except:
                    pass
            ...
            if hasattr(self.engine, '_request_tracker'):
                for request_id in list(self.engine._request_tracker._request_streams.keys()):
                    self.engine._abort(request_id)
                    
            if hasattr(self.engine, 'engine') and hasattr(self.engine.engine, 'workers'):
...
if __name__ == "__main__":
    ...
    sampling_params = SamplingParams(temperature=0.9, top_p=0.95,max_tokens=max_tokens,support_partial_output=True,support_partial_prefilling=True)
    ...
    ray.get(llm_engine.submit_request.remote("a", "llm::a", "1a2b3c", prompt[:int(len(prompt)*0.8)], NodeOps.LLM_PARTIAL_PREFILLING, -1, sampling_params))

    time.sleep(3)
    print(f"begin full prefilling")
    ray.get(llm_engine.submit_request.remote("b", "llm::b", "1a2b3c", prompt[int(len(prompt)*0.8):], NodeOps.LLM_FULL_PREFILLING, -1, sampling_params))

    #ray.get(llm_engine.submit_request.remote("1", "1", "xxx", prompt, NodeOps.LLM_DECODING, -1, sampling_params))
    ray.get(llm_engine.submit_request.remote("c", "llm::c", "1a2b3c", prompt, NodeOps.LLM_PARTIAL_DECODING, 0, sampling_params))
    ray.get(llm_engine.submit_request.remote("d", "llm::d", "1a2b3c", prompt, NodeOps.LLM_PARTIAL_DECODING, 1, sampling_params))
    ray.get(llm_engine.submit_request.remote("e", "llm::e", "1a2b3c", prompt, NodeOps.LLM_PARTIAL_DECODING, 2, sampling_params))
    ray.get(llm_engine.submit_request.remote("f", "llm::f", "1a2b3c", prompt, NodeOps.LLM_PARTIAL_DECODING, 3, sampling_params))
    ray.get(llm_engine.submit_request.remote("g", "llm::g", "1a2b3c", prompt, NodeOps.LLM_PARTIAL_DECODING, 4, sampling_params))
