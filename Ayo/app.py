...
from asyncio import Queue, Future, Lock, Semaphore
...
class APP:
    ...
    def register_engine(self, config: EngineConfig) -> None:
        ...
        try:
            ...
            scheduler_ready = ray.get(scheduler_actor.is_ready.remote())

            logger.info(f"Registered engine {engine_type} with scheduler {scheduler_actor}")
            
            self.engine_schedulers[engine_type] = scheduler_actor
            
            if self.graph_scheduler:
                ray.get(self.graph_scheduler.update_schedulers.remote(self.engine_schedulers))
    ...
    async def submit_query(self, 
        ...
        logger.info(f"Query enqueued: query_id={query.query_id}, queue_size={self.query_queue.qsize()}")
        return future
        ...
        while self.is_running:
            try:
                try:
                    ...
                    logger.info(f"Retrieved query from queue: query_id={query_task.query.query_id}")
                ...
                if time() - query_task.timestamp > query_task.timeout:
                    logger.warning(f"Query timeout detected: query_id={query_task.query.query_id}")
                    await self.handle_timeout(query_task)
                    self.query_queue.task_done()
                    continue

                logger.info(f"Starting query processing: query_id={query_task.query.query_id}")
    ...
    async def _monitor_query_status(self, query_task: QueryTask, query_id: str):
        ...
        try:
            ...
            while True:
                ...
                status =ray.get(self.graph_scheduler.get_query_status.remote(query_task.query.query_id))
                ...
                if status == QueryStatus.COMPLETED:
                    ...
                    logger.info(f"Query completed successfully: query_id={task_id}, duration={latency:.3f}s")
                    print(f"query_task.query.results: {query_task.query.results}")
                 
                    node_results=ray.get(query_task.query.query_state.get_node_results.remote())
            ...
            logger.error(f"Error in query monitoring: query_id={task_id}, error={str(e)}")
            await self.handle_error(query_task, e)
        finally:
            async with self.query_lock:
        ...
        while not self.query_queue.empty():
            try:
                ...
                task.future.set_exception(
                    RuntimeError("Application shutting down")
                )
