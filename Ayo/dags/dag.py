...
import ray
import uuid
from Ayo.dags.node import Node, NodeIOSchema, NodeType, EngineType
...
class DAG:
    ...
    def create_input_nodes(self) -> None:
        ...
        for input_name, input_value in self._query_inputs.items():
            ...
            for node in self.nodes:
                if (node.node_type == NodeType.COMPUTE and 
                    input_name in node.input_kwargs):
    ...
    def optimize(self, passes: List[OPT_Pass]) -> 'DAG':
        ...
        for opt_pass in passes:
            ...
            try:
                opt_pass.log_optimization(f"Starting optimization")
                opt_pass.run(self)
                opt_pass.log_optimization(f"Optimization completed")
            except Exception as e :
                import traceback
                stack_trace = traceback.format_exc()
                opt_pass.log_optimization(f"Optimization failed: {str(e)}\nStack trace: {stack_trace}")
                continue
        ...
        try:
            ...
            for node in self.nodes:
                ...
                if node.node_type == NodeType.INPUT and node.parents:
                    raise ValueError(f"Input node {node.name} should not have parent nodes")
                if node.node_type == NodeType.OUTPUT and node.children:
                    raise ValueError(f"Output node {node.name} should not have child nodes")
    ...
    def get_execution_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'successful_nodes': sum(1 for node in self.nodes 
