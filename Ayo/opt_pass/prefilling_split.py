...
from typing import List, Dict, Set, TYPE_CHECKING, Optional, Any, Tuple
import re
...
class PrefillingSpiltPass(OPT_Pass):
    ...
    def _is_splittable_prefilling(self, node: Node) -> bool:
        ...
        if node.op_type == NodeOps.LLM_PREFILLING:
            ...
            for parent in parents:
                if parent.name in node.input_key_from_parents and abs(node.depth - parent.depth) > 1:
                    return True
    ...
    def _analyze_template(self, template: str) -> Tuple[str, str, List[str], List[str]]:
        ...
        for placeholder in unique_placeholders:
            ...
            pos = template.find(placeholder_pattern, split_pos + len(f"{{{split_placeholder}}}"))
        ...
        if next_placeholder_pos == -1:
            logger.warning(f"Cannot find next placeholder for {split_placeholder} in template")
            end_markers = ['.', '?', '!']
            end_pos = -1
        ...
        # Split the template
        partial_template = template[:end_pos]  # Include the content before the second placeholder
        full_template = template[end_pos:]  # Include the content from the first placeholder
    ...
    def _split_prefilling(self, dag: DAG, node: Node, processed_nodes: Set[Node]) -> None:
        ...
        """
        # Get the template
        template = node.config.get("template", None) or node.config.get("prompt_template", None)
        ...
        partial_template, full_template, partial_deps, full_deps = self._analyze_template(template)
        
        if not full_template or not partial_deps:
            # If it cannot be split, do not process
            logger.warning(f"Cannot split prefilling node {node.name} because it cannot be split")
        ...
        partial_io_schema.input_format = {
            k: v for k, v in node.io_schema.input_format.items() 
            if k in partial_deps
        }
        ...
        for parent in node.parents:
            ...
            logger.info(f"original prefilling's parent: {parent.name}, input_key: {input_key}")
            if input_key in partial_deps:
                ...
                raise ValueError(f"Cannot find similar dependency for {input_key} in {full_deps+partial_deps}")
        ...
        for child in node.children:
            ...
            for output_key in node.io_schema.output_format:
                if node.name in child.input_key_from_parents and child.input_key_from_parents[node.name] == output_key:
...
if __name__ == "__main__":
    ...
    search_node = Node(
        ...
        io_schema=NodeIOSchema(
            input_format={"question": str},
            output_format={"context": str}
        ),
        config={
        }
    )
    ...
    decoding_node = Node(
        ...
        io_schema=NodeIOSchema(
            input_format={"prefilled_done": bool},
    ...
    for node in dag.nodes:
        if node.op_type in [NodeOps.LLM_PARTIAL_PREFILLING, NodeOps.LLM_FULL_PREFILLING]: 
