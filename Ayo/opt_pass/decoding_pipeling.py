...
class LLMDecodingPipeliningPass(OPT_Pass):
    ...
    def _is_splittable_llm_decoding(self, node: "Node") -> bool:
        ...
        """
        if (
            node.op_type == NodeOps.LLM_DECODING
            and node.children[0].op_type in self.batchable_ops
        ):
    ...
    def _is_pipeline_end(self, node: "Node") -> bool:
        ...
        """
        if (
            node.op_type in self.batchable_ops
            and node.op_type not in self.splittable_ops
        ):
            return True
        return False
    ...
    def _split_pipeline(
        self, dag: "DAG", start_node: "Node", processed_nodes: Set["Node"]
    ) -> None:
        ...
        while True:
            ...
            num_sub_nodes = (
                self.num_splits if current_sub_nodes is None else len(current_sub_nodes)
            )
            sub_nodes = []
            ...
            if current_sub_nodes is None:
                ...
                other_parents = [
                    p for p in current_node.parents if p != sub_nodes[0].parents[0]
                ]
            ...
            if not self._is_in_pipeline(current_node) and self._is_pipeline_end(
                current_node
            ):
                logger.warning(
                    f"encounter a node at the end of pipeline: {current_node.name}"
                )
    ...
    def _create_split_io_schema(self, original_schema, split_idx: int) -> NodeIOSchema:
        ...
        new_schema.output_format = {
            f"{k}_{split_idx}": v for k, v in original_schema.output_format.items()
        }
        return new_schema

    def _create_aggregator_node(
        self, dag: "DAG", sub_nodes: List["Node"], next_node: "Node"
    ) -> "Node":
    ...
    def _create_aggregator_schema(
        self, sub_nodes: List["Node"], next_node: "Node"
    ) -> "NodeIOSchema":
        ...
        return NodeIOSchema(
            input_format=input_format, output_format=next_node.io_schema.input_format
        )
    ...
    def _connect_aggregator(
        self, dag: "DAG", sub_nodes: List["Node"], agg_node: "Node", next_node: "Node"
    ):
        """Connect the aggregator node"""
...
if __name__ == "__main__":
    ...
    dag.set_query_inputs({"prefilled": True, "query": "What is the capital of France?"})
    ...
    prefilling_node = Node(
        ...
        io_schema=NodeIOSchema(
            input_format={"query": str}, output_format={"prefilled": bool}
        ),
