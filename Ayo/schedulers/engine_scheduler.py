...
from Ayo.dags.node import NodeOps
...
class SchedulingStrategy(ABC):
    ...
    @abstractmethod
    def get_next_engine(self, engines: List[Any], current_idx: int) -> 'tuple[Any, int]':
...
class EngineScheduler(BaseEngineScheduler):
    ...
    def _create_engines(self):
        ...
        for i in range(self.num_instances):
            ...
            logger.info(f"try to create engine instance with name: {self.name}_{i} for class: {self.engine_class} with options: {options}")
            ...
            logger.info(f"created engine instance with name: {self.name}_{i} for class: {self.engine_class}")
    ...
    async def _submit_requests(self):
        ...
        while self.running:
            try:
                ...
                logger.info(f"submit request {request.request_id} with payload: {request.payload.keys()}")
                    ...
                    try:
                        ...
                        await asyncio.get_event_loop().run_in_executor(
                            ...
                            query_states.set_node_result.remote(
                                request.request_id.split("::")[-1], # here we assume the request_id is in the format of {query_id}::{node_name} as in the graph scheduler submit_node method
                                request.result_ref
                            )
                        )
                        
                        logger.info(f"Successfully set result for request {request.request_id}")
                        
                    except Exception as e:
                        logger.error(f"Error processing result for request {request.request_id}: {e}")
    ...
    async def on_result(self, request_id: str, query_id: str, result_ref_from_engine: Any):
