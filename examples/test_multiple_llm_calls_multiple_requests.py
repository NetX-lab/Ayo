...
async def run_app_multiple_requests(dags):

    request_lens = len(dags)
    ...
    async def delayed_query(query_text, index, dag):
        if index > 0:
    ...
    tasks = [delayed_query(test_queries[i], i, dags[i]) for i in range(request_lens)]
...
if __name__ == "__main__":
    ...
    dags = [create_base_dag() for _ in range(total_requests)]
    ...
    # Run the application
    asyncio.run(run_app_multiple_requests(dags))
