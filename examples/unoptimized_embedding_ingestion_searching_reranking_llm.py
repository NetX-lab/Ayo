...
def create_base_dag():
    ...
    llm_prefilling_node = Node(
        ...
        anno=NodeAnnotation.BATCHABLE,
        config={
            'prompt_template': replace_placeholders(RAG_QUESTION_ANSWERING_PROMPT_TEMPLATE_STRING, question="queries", context="search_results"),
            'parse_json': True, 
            'prompt':replace_placeholders(RAG_QUESTION_ANSWERING_PROMPT_TEMPLATE_STRING, question="queries", context="search_results"),
...
if __name__ == "__main__":
    from Ayo.vis.vis_graph import visualize_dag_with_node_types, visualize_dag_with_compute_nodes_in_line
    
    dag = create_base_dag()

    print(dag.get_full_dag_nodes_info())

    visualize_dag_with_compute_nodes_in_line(dag, "unoptimized_dag_for_embedding_ingestion_search_reranking_llm.png")
