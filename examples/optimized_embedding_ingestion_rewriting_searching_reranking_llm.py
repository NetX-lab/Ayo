...
# Create vector database engine config
...
reranker_config = EngineConfig(
    ...
    num_cpus=1,
    instances=1,
    model_config={**default_reranker_config, "device": "cuda"},
    latency_profile={"timeout": 60, "batch_wait": 0.1},
)
...
def create_base_dag():
    """Create basic DAG"""
    base_dag = DAG(
        dag_id="optimized_embedding_ingestion_rewriting_searching_reranking_llm"
    )
    ...
    expanded_query_prefilling_node = Node(
        ...
        io_schema=NodeIOSchema(
            input_format={"queries": str},
            output_format={"expanded_query_prefilled": bool},
        ),
        anno=NodeAnnotation.BATCHABLE,
        config={
    ...
    search_node = Node(
        ...
        config={
            "top_k": 20
        },  # Add more retrieval to provide more candidates for reranking
    )
    ...
    reranking_node = Node(
        ...
        io_schema=NodeIOSchema(
            input_format={
                "queries": str,
                "search_results": Union[List[List[str]], List[str]],
            },
    ...
    llm_decoding_node = Node(
        ...
        engine_type=EngineType.LLM,
        io_schema=NodeIOSchema(
            input_format={"prefill_state": bool}, output_format={"result": str}
        ),
        op_type=NodeOps.LLM_DECODING,
        anno=NodeAnnotation.BATCHABLE,
        config={
    ...
    base_dag.set_query_inputs(
        {
            ...
            * 60
            ...
            "queries": ["I want to know some system conferences."],
        },
    )
...
if __name__ == "__main__":
    ...
    dag.set_dag_id(
        "optimized_dag_for_embedding_ingestion_rewriting_searching_reranking_llm"
    )

    visualize_dag_with_node_types(
        dag,
        "optimized_dag_for_embedding_ingestion_rewriting_searching_reranking_llm.png",
    )
